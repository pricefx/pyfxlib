...
poetry run flake8 pyfx &&\
    poetry run flake8 --config .flake8-tests tests &&\
    poetry run mypy pyfx --exclude _testtooling &&
    poetry run pytest tests/unit &&\
    if nc -z localhost 2000; then
        poetry run pytest tests/integration -m "not extended"
    else
        echo "ERROR: pyfx local instance not running on localhost:2000. To run a local instance, execute:" >&2
